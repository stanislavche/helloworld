...
class VM {
  constructor() {
    ...
    this.pal = document.getElementById('palette').value;
    this.rewind = {
      minTicks: 0,
      maxTicks: 0,
    };
...
class Emulator {
  ...
  keyDown(event) {
    this.module._set_joyp_start(null, this.e);
...
class Audio {
  ...
  pushBuffer() {
    ...
    if (this.startSec >= nowSec || customVolume !== vm.volume) {
      vm.volume = customVolume;
...
document.getElementById('palette').addEventListener('change', function (event) {
  emulator.setBuiltinPalette(this.value);
})
...
// document.getElementById('fullscreenMode').addEventListener('click', function (event) {
//   event.preventDefault();
...
const keup = new KeyboardEvent('keyup', {
  bubbles: true, cancelable: true, keyCode: 13, isTrusted: true, key: "Enter", code: "Enter"
});
...
window.addEventListener('click', initEmulator);
window.addEventListener('keydown', initEmulator);
